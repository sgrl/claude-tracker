...
A native macOS menubar app that surfaces [Claude Code](https://claude.com/claude-code) usage in real time — rate-limit windows, estimated cost, per-project / model / session breakdowns, and a dedicated detail view for each session.

## What it shows

- **5-hour block** and **7-day window** — live percentages and reset times, pulled straight from Claude Code's own rate-limit payload.
- **Today** and **this week** — token + cost totals, plus a per-hour (today) and per-day (week) sparkline.
...
- **By project / model / session** — Today / 7 days / All-time scope toggles; expandable rows.
...
The cask's `postflight` step removes the quarantine attribute automatically.

### Manual

Download the latest `.dmg` from [Releases](https://github.com/sgrl/claude-tracker/releases), drag `claudetracker.app` into `/Applications/`, then remove the quarantine attribute:

```
xattr -dr com.apple.quarantine /Applications/claudetracker.app
```

### Why the unquarantine step?

Claude Tracker is currently ad-hoc signed (no Apple Developer account on file), so macOS Gatekeeper refuses to launch it until the quarantine attribute is removed. The command above does that once; after that it launches normally. If/when the project picks up a Developer ID certificate, this step will go away.
